...
def add_knowledge():
    return (
        ...
        " A decision graph is a directed acyclic graph (DAG) with implicitly defined start and end points. Nodes"
        " with no incoming edges are automatically identified as start nodes, and nodes with no outgoing"
        " edges are identified as end nodes. Each tuple in the `dependencies` list represents a"
        " sequential pair. For example, a list of tuples `[(A, B), (A, C)]` will create the directed edges"
        " `A -> B` and `A -> C`."
        ...
        " where exactly one path can be executed."
        " Assume we have 4 submodels A, B, C, D."
        " - `decision_graph(dependencies=[(A, C), (B, D), (B, C)])` models the following sequences (paths):"
        ...
        " - `decision_graph(dependencies=[(A, B), (A, C), (A,D), (B,E), (C,E), (D,E)])` models the"
        " following sequences: A->B->E, A->C->E, A->D->E. As per definition, only one one of these"
        " will be executed. IMPORTANT: a decision graph can express simple sequential dependencies, e.g., A is followed by B."
...
def negative_prompting():
    return (
        ...
        "```\n\n"
        " A decision graph must be a Directed Acyclic Graph (DAG). Providing dependencies that create"
        " a cycle (e.g., `[(A, B), (B, A)]`) will lead to an error:"

        "```python\n"
        "decision_graph = decision_graph(dependencies=[(A, B), (B, A)]`))"
        "```\n\n"
        "When using decision graphs, **keep the model as flat as possible**"
        "Examples of unflattened model::"
        "```python\n"
        "decision_graph = `decision_graph(dependencies=[(A, C), (B,D), partial_order[dependencies=(C, D)]])`\n"
        "```\n\n"
        "Instead, do this:"
        "```python\n"
        "decision_graph = `decision_graph(dependencies=[(A, C), (B,D), (C, D)])`\n"
        "```\n\n"
        "Examples of unflattened model:"
        "```python\n"
        "activity_block = Transition(A)"
        "decision_graph = `decision_graph(dependencies=[(B, D), (C, D)])`\n"
        "model = partial_order(dependencies(activity_block, decision_graph))`\n"
        "```\n\n"
        "Instead, do this:"
        "```python\n"
        "decision_graph = `decision_graph(dependencies=[(A, B), (A, C), (B, D), (C, D)])`\n"
    )
