...
def add_knowledge():
    return (
        ...
        " of POWL models, we use a directed graph structure to combine"
        " multiple POWL models into a new model. Directed graphs can model complex control flow, where exactly one path is taken at a time."
        " They can further model various control flow patterns: a do-redo loop and exclusive choice structures"
        " of n >= 2 sub-models."
        ...
        " A decision graph is a directed acyclic graph (DAG) with implicitly defined start and end points."
        ...
        " where exactly one path can be executed. However, they cannot model concurrency."
        " Assume we have 4 submodels A, B, C, D."
        " - `decision_graph(dependencies=[(None, A), (A, C), (B, D), (B, C), (C, None), (D, None)])` models the following sequences (paths):"
        " A-> C, B->C, B->D where exactly one of these will be executed, in the end."
        " A is the only start node and C and D are the possible end nodes."
        " Decision graphs can also model do-redo loops. For example, "
        " `decision_graph(dependencies=[(None, A), (A, B), (B, A), (A, None)])` models a do-redo loop between A and B,"
        " where A is the start and end node. The cycle (A, B), (B, A) means that you always start with A, then you can go to B, then back to A, and so on."
        " Decision graphs can also model exclusive choice structures of n >= 2 sub-models. For example, "
        " `decision_graph(dependencies=[(None, A), (A, B), (A, C), (B, None), (C, None)])` models an exclusive choice between B and C."
        ...
        "IMPORTANT: When using a decision graph, try to flatten the model by integrating partial orders with decision graphs. \n\n"
        "Additionally, we define two helper functions:\n"
        " - skip(submodel) takes 1 argument, which is a submodel. It makes the submodel optional/skippable.\n"
        " - self_loop(submodel) takes 1 argument, which is a submodel. It creates a self-loop on the submodel."
        "The subprocess must be executed at least once but can be repeated multiple times afterwards.\n"
...
def negative_prompting():
    return (
        ...
        " a cycle (e.g., `[(None, A), (A, B), (B, A), (B, None)]`) means that you always start with B, then you can go to A, then back to B, and so on." \
